...
function Mamba(host, port, eventListener, verboseLogging) {
    ...
    if (!eventListener) {
        logError(
            "Missing event listener...so I'm supposed to talk to the hand yeah?"
        );
        return null;
    }
  ...
  function handleRegistrationDone(json) {
    ...
    log(
      `Registration complete - ${player.getPlayerName()} (id:${player.getPlayerId()}) is now registered on ${player.getGameId()}`
    );
    nextState();
  ...
  function handleGameEvent(json) {
    ...
    if (json.type === MapUpdateEvent.type) {
      ...
      event = {
        type: "GAME_MAP_UPDATED",
        payload: MapUpdateEvent.create(json)
      };
      ...
      event = {
        ...
        payload: TournamentEndedEvent.create(json)
      };
    } else if (json.type === GameStartingEvent.type) {
      event = {
        type: "NEW_GAME_STARTED",
        payload: GameStartingEvent.create(json)
      };
